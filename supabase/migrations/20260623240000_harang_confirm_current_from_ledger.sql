-- 재고조정 확정: LOT 현재고 = 원장 합산 (실사일 이후 타 제품·전체 usage 반영)
-- 기존: current = 실재고 − (이번 사이클 생산만) → 6/2 이후 사용이 무시됨
-- 적용 후: SELECT harang_sync_inventory_lots_from_ledger() 로 기존 LOT 현재고 보정
