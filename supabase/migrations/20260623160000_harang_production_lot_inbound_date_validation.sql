-- 생산일 이전에 입고된 LOT만 생산입고에 사용 가능하도록 검증 + 기존 위반 이력 조회 RPC
...
CREATE OR REPLACE FUNCTION public.harang_list_production_lot_inbound_violations()
RETURNS TABLE (
...
AS $$
BEGIN
  IF NOT public.can_access_harang_data() THEN
    RAISE EXCEPTION '권한이 없습니다.';
  END IF;
  ...
  JOIN public.harang_production_headers h ON h.id = pl.header_id
  JOIN public.harang_inventory_lots l ON l.id = pll.lot_id
  WHERE l.inbound_date IS NOT NULL
    AND l.inbound_date > h.production_date
  ORDER BY h.production_date DESC, h.production_no, pl.sort_order, l.lot_date;
END;
$$;

GRANT EXECUTE ON FUNCTION public.harang_list_production_lot_inbound_violations() TO authenticated;
