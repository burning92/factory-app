-- Fix confirm: physical is as-of adjustment_date; add post-date usage for actual_usage, delta distribution, and final inventory
...
BEGIN
  ...
  LOOP
    ...
    WHERE t.lot_id = ANY(v_serial.lot_ids)
      AND t.tx_type = 'usage'
      AND t.tx_date > v_session.adjustment_date;

    -- 실사일 잔량 + 실사일 이후 사용 = 사이클 종료 시점 실사 사용량 역산
    v_actual := v_inbound - v_physical + v_usage_after_adj;
    ...
    LOOP
      ...
      -- 실사일 이후 이미 기록된 사용(생산입고)만큼 차감 — 실사 수량은 조정일 기준 잔량
      SELECT COALESCE(SUM(-t.quantity_delta), 0)::NUMERIC(14, 3) INTO v_usage_after_adj
      FROM public.harang_inventory_transactions t
      ...
      v_new_qty := GREATEST(0, v_new_qty - v_usage_after_adj);
