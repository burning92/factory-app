-- Production cycle stock adjustment confirm + result snapshot (consumption ratio)

CREATE TABLE IF NOT EXISTS public.harang_stock_adjustment_serial_results (
  ...
  system_stock_qty NUMERIC(14, 3) NOT NULL,
  bom_usage_qty NUMERIC(14, 3) NOT NULL DEFAULT 0,
  actual_usage_qty NUMERIC(14, 3) NOT NULL,
  usage_delta_qty NUMERIC(14, 3) NOT NULL DEFAULT 0,
  consumption_ratio_pct NUMERIC(10, 2),
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  UNIQUE (session_id, material_category, material_id, lot_date)
);

CREATE TABLE IF NOT EXISTS public.harang_stock_adjustment_production_deltas (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  session_id UUID NOT NULL REFERENCES public.harang_stock_adjustment_sessions(id) ON DELETE CASCADE,
  serial_result_id UUID NOT NULL REFERENCES public.harang_stock_adjustment_serial_results(id) ON DELETE CASCADE,
  production_header_id UUID NOT NULL REFERENCES public.harang_production_headers(id) ON DELETE RESTRICT,
  usage_delta_qty NUMERIC(14, 3) NOT NULL,
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  UNIQUE (serial_result_id, production_header_id)
);

CREATE INDEX IF NOT EXISTS idx_harang_stock_adj_serial_results_session
  ON public.harang_stock_adjustment_serial_results (session_id);

CREATE INDEX IF NOT EXISTS idx_harang_stock_adj_prod_deltas_session
  ON public.harang_stock_adjustment_production_deltas (session_id);

ALTER TABLE public.harang_stock_adjustment_serial_results ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.harang_stock_adjustment_production_deltas ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "harang_stock_adj_serial_results_select" ON public.harang_stock_adjustment_serial_results;
DROP POLICY IF EXISTS "harang_stock_adj_serial_results_write" ON public.harang_stock_adjustment_serial_results;
CREATE POLICY "harang_stock_adj_serial_results_select"
  ON public.harang_stock_adjustment_serial_results FOR SELECT TO authenticated
  USING (public.can_access_harang_data() OR public.is_headquarters_organization());
CREATE POLICY "harang_stock_adj_serial_results_write"
  ON public.harang_stock_adjustment_serial_results FOR ALL TO authenticated
  USING (public.can_write_harang_stock_adjustment())
  WITH CHECK (public.can_write_harang_stock_adjustment());

DROP POLICY IF EXISTS "harang_stock_adj_prod_deltas_select" ON public.harang_stock_adjustment_production_deltas;
DROP POLICY IF EXISTS "harang_stock_adj_prod_deltas_write" ON public.harang_stock_adjustment_production_deltas;
CREATE POLICY "harang_stock_adj_prod_deltas_select"
  ON public.harang_stock_adjustment_production_deltas FOR SELECT TO authenticated
  USING (public.can_access_harang_data() OR public.is_headquarters_organization());
CREATE POLICY "harang_stock_adj_prod_deltas_write"
  ON public.harang_stock_adjustment_production_deltas FOR ALL TO authenticated
  USING (public.can_write_harang_stock_adjustment())
  WITH CHECK (public.can_write_harang_stock_adjustment());
...
BEGIN
  ...
  FOR v_serial IN
    ...
    WHERE lp.session_id = p_session_id
      AND l.inbound_date <= v_session.adjustment_date
  ...
  LOOP
    ...
    IF v_target_delta <> 0 THEN
      v_assigned := 0;
      v_n := array_length(v_sorted_headers, 1);

      FOR v_i IN 1..v_n LOOP
        SELECT h.id, h.finished_qty INTO v_header
        FROM public.harang_production_headers h
        WHERE h.id = v_sorted_headers[v_i];

        IF v_i = v_n THEN
          v_hdr_delta := v_target_delta - v_assigned;
        ELSE
          v_hdr_delta := round((v_target_delta * v_header.finished_qty) / v_total_finished)::INTEGER;
        ...
        SELECT ln.id, ln.usage_qty INTO v_line
