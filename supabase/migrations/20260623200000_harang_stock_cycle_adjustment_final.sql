-- 생산 사이클 재고조정 최종 RPC (통합본)
-- · harang_inventory_stock_as_of(_lot): 기준일 재고, usage 이중집계 방지, lot_id ambiguous 수정
-- · confirm: BOM = 원장에 실제 차감된 생산 line.bom_qty 합, 미사용 소비기한 LOT 제외
...
BEGIN
  ...
  keys AS (
    ...
    UNION
    SELECT lot_id FROM prod_usage_after
  )
  SELECT
    k.lot_id,
    l.category,
    ...
    (
      ...
      + GREATEST(
          0::NUMERIC(14, 3),
          COALESCE(p.qty, 0) - COALESCE(u.qty, 0)
        )
  ...
  LEFT JOIN prod_usage_after p ON p.lot_id = k.lot_id
  LEFT JOIN usage_tx_after u ON u.lot_id = k.lot_id;
END;
$$;
...
BEGIN
  ...
  keys AS (
    ...
    UNION
    SELECT category, item_id FROM prod_usage_after
  )
  SELECT
    k.category,
    k.item_id,
  ...
  LEFT JOIN prod_usage_after p ON p.category = k.category AND p.item_id = k.item_id
  LEFT JOIN usage_tx_after u ON u.category = k.category AND u.item_id = k.item_id;
END;
$$;
