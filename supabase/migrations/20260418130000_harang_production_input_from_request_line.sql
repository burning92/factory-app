-- ============================================================
-- Harang production input -> request line execution binding
-- - header link columns: request_id / request_line_id
-- - RPC: create_harang_production_from_request_line
-- ============================================================

ALTER TABLE public.harang_production_headers
  ADD COLUMN IF NOT EXISTS request_id UUID REFERENCES public.harang_production_requests(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS request_line_id UUID REFERENCES public.harang_production_request_lines(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_harang_production_headers_request_id
  ON public.harang_production_headers (request_id);

CREATE INDEX IF NOT EXISTS idx_harang_production_headers_request_line_id
  ON public.harang_production_headers (request_line_id);
...
BEGIN
  ...
  END IF;
  IF NOT (public.is_harang_organization() OR public.get_my_profile_role() = 'admin') THEN
    RAISE EXCEPTION '생산입력은 하랑(조직 200) 또는 관리자만 가능합니다.';
  END IF;
  ...
  v_rem := COALESCE(v_req.remaining_qty, 0);
  IF p_finished_qty > v_rem THEN
    RAISE EXCEPTION '이번 생산수량이 잔여수량을 초과합니다.';
  END IF;
  ...
  INSERT INTO public.harang_production_headers (
    production_date, production_no, product_name, finished_qty, note, created_by, request_id, request_line_id
  )
  VALUES (
    p_production_date,
    v_production_no,
  ...
  LOOP
    ...
    IF NOT EXISTS (
      ...
      WHERE m.request_line_id = p_request_line_id
        AND m.material_category = v_material_category
        AND m.material_id = v_material_id
    ) THEN
      RAISE EXCEPTION '작업지시 BOM 스냅샷에 없는 소모품목입니다: %', v_material_name;
    END IF;
  ...
  SET
    produced_qty = produced_qty + p_finished_qty,
    remaining_qty = remaining_qty - p_finished_qty,
    updated_at = now()
