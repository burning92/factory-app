...
export default function AccountHubPage() {
  const { viewOrganizationCode, organization } = useAuth();
  const isHarangOrg = organization?.organization_code === "200";
  const showExecutive = !isHarangOrg && viewOrganizationCode !== "200";
  const linkItems = isHarangOrg
    ? LINK_ITEMS.filter((item) => item.href !== "/account/leave")
    : [...LINK_ITEMS];
  ...
  return (
    <div className="min-h-[calc(100vh-3.5rem)] md:min-h-0 p-4 md:p-6 max-w-2xl mx-auto">
      ...
      <p className="text-slate-500 text-sm mb-4">
        {isHarangOrg ? "비밀번호·로그아웃" : "비밀번호·로그아웃·개인정보"}
      </p>
