...
/**
 * 영업장 온·습도 점검일지 — 목록 전용.
 * 입력: /daily/temperature-humidity/new, 수정: /[id]/edit, 상세: /[id]
 */
...
export default function DailyTempHumidityListPage() {
  ...
  const handleDelete = async (logId: string) => {
    ...
    setDeletingId(logId);
    setToast(null);
    const { error } = await supabase.from("daily_temp_humidity_logs").delete().eq("id", logId);
    setDeletingId(null);
    if (error) {
      setToast({ message: error.message, error: true });
      return;
  ...
  return (
    <div className="min-h-[calc(100vh-3.5rem)] md:min-h-0 p-4 md:p-6 max-w-2xl mx-auto pb-20 md:pb-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-slate-200 font-medium">영업장 온·습도 점검일지</span>
      </div>

      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h1 className="text-lg font-semibold text-slate-100">영업장 온·습도 점검일지 — 목록</h1>
        <Link
          href="/daily/temperature-humidity/new"
