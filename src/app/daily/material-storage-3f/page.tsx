...
export default function DailyMaterialStorage3fListPage() {
  ...
  const getRowHref = (log: LogRow) => {
    ...
    if (log.status === "approved" && isManager) return `/daily/material-storage-3f/${log.id}/edit`;
    return `/daily/material-storage-3f/${log.id}`;
  };

  const getEditHref = (log: LogRow) => {
    if (log.status === "submitted") return `/daily/material-storage-3f/${log.id}`;
    return `/daily/material-storage-3f/${log.id}/edit`;
  };
