...
export default function DailyMaterialStorage3fListPage() {
  ...
  const fetchLogs = useCallback(async () => {
    ...
    const { data, error } = await supabase
      ...
      .select("id, inspection_date, author_name, status, approved_at, approved_by_name, reject_reason")
  ...
  const getRowHref = (log: LogRow) => {
    if (log.status === "draft" || log.status === "rejected") return `/daily/material-storage-3f/${log.id}/edit`;
    return `/daily/material-storage-3f/${log.id}`;
  };
  ...
  const handleDelete = async (logId: string) => {
    ...
    setDeletingId(logId);
    setToast(null);
    const { error } = await supabase.from("daily_material_storage_3f_logs").delete().eq("id", logId);
    setDeletingId(null);
    if (error) {
      setToast({ message: error.message, error: true });
      return;
  ...
  return (
    <div className="min-h-[calc(100vh-3.5rem)] md:min-h-0 p-4 md:p-6 max-w-2xl mx-auto pb-20 md:pb-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-slate-200 font-medium">원부자재 창고 점검표(3F)</span>
      </div>

      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h1 className="text-lg font-semibold text-slate-100">원부자재 창고 점검표(3F) — 목록</h1>
        <Link href="/daily/material-storage-3f/new" className="shrink-0 inline-flex items-center justify-center px-4 py-2.5 rounded-lg bg-cyan-600 hover:bg-cyan-500 text-white font-medium text-sm">
          새 일지 작성
        </Link>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-slate-700/60 bg-slate-800/30 p-8 text-center">
          ...
          <Link href="/daily/material-storage-3f/new" className="inline-flex items-center justify-center px-4 py-2 rounded-lg bg-cyan-600 hover:bg-cyan-500 text-white text-sm font-medium">
            새 일지 작성
          </Link>
        </div>
      ) : (
              ...
              <Link href={getRowHref(log)} className={`block px-4 py-3 rounded-xl border transition-colors ${log.status === "submitted" ? "border-cyan-600/50 bg-slate-800/80 hover:bg-slate-700/60" : "border-slate-700/60 bg-slate-800/50 hover:bg-slate-700/50"}`}>
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                      <button type="button" onClick={(e) => { e.preventDefault(); e.stopPropagation(); window.location.href = `/daily/material-storage-3f/${log.id}/edit`; }} className="inline-flex items-center justify-center w-9 h-9 rounded-lg border border-slate-700/60 bg-slate-900/30 hover:bg-slate-700/40 text-slate-200" title="수정" aria-label="수정">
