...
export default function DailyColdStorageHygieneListPage() {
  ...
  const fetchLogs = useCallback(async () => {
    ...
    const { data, error } = await supabase
      ...
      .select("id, inspection_date, author_name, status, approved_at, approved_by_name, reject_reason, created_at")
  ...
  const getRowHref = (log: LogRow) => {
    if (log.status === "draft" || log.status === "rejected") return `/daily/cold-storage-hygiene/${log.id}/edit`;
    if (log.status === "approved" && isManager) return `/daily/cold-storage-hygiene/${log.id}/edit`;
    return `/daily/cold-storage-hygiene/${log.id}`;
  };

  const getEditHref = (log: LogRow) => {
    if (log.status === "submitted") return `/daily/cold-storage-hygiene/${log.id}`;
    return `/daily/cold-storage-hygiene/${log.id}/edit`;
  };

  const handleDelete = async (logId: string) => {
    if (!isManager) return;
    const ok = window.confirm("이 점검일지를 삭제할까요? (항목·온도 데이터도 함께 삭제됩니다)");
    if (!ok) return;
    setDeletingId(logId);
    setToast(null);
    const { error } = await supabase.from("daily_cold_storage_hygiene_logs").delete().eq("id", logId);
    setDeletingId(null);
    if (error) {
      setToast({ message: error.message, error: true });
      return;
  ...
  return (
    <div className="min-h-[calc(100vh-3.5rem)] md:min-h-0 p-4 md:p-6 max-w-2xl mx-auto pb-20 md:pb-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-slate-200 font-medium">냉장 · 냉동온도 및 위생 점검일지</span>
      </div>

      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h1 className="text-lg font-semibold text-slate-100">냉장 · 냉동온도 및 위생 점검일지</h1>
        <Link
          href="/daily/cold-storage-hygiene/new"
