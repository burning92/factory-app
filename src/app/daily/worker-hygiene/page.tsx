...
/** 작업자 위생 점검일지 — 목록 (조직·일자당 1건, 담당자 종합 기록) */
...
export default function DailyWorkerHygieneListPage() {
  ...
  const handleDelete = async (logId: string) => {
    ...
    setDeletingId(logId);
    setToast(null);
    const { error } = await supabase.from("daily_worker_hygiene_logs").delete().eq("id", logId);
    setDeletingId(null);
    if (error) {
      setToast({ message: error.message, error: true });
      return;
  ...
  return (
    <div className="min-h-[calc(100vh-3.5rem)] md:min-h-0 p-4 md:p-6 max-w-2xl mx-auto pb-20 md:pb-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-slate-200 font-medium">작업자 위생 점검일지</span>
      </div>

      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div className="min-w-0">
          <h1 className="text-lg font-semibold text-slate-100">작업자 위생 점검일지 — 목록</h1>
          <p className="text-slate-500 text-sm mt-1 max-w-xl">
            점검일자·조직당 하루 1건. 대표 담당자가 작성하는 현장 종합 점검 기록입니다.
          </p>
        </div>
        <Link
          href="/daily/worker-hygiene/new"
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2 gap-y-1">
                      ...
                      {log.author_name && (
                        <span className="text-slate-500 text-sm">담당 작성: {log.author_name}</span>
                      )}
                      <span
