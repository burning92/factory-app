...
export default function DailyWorkerHygieneViewPage() {
  ...
  items.forEach((i) => {
    const cat = normalizeWorkerHygieneCategoryTitle(i.category);
    const key = `${cat}-${i.question_index}`;
  ...
  return (
    <div className="min-h-[calc(100vh-3.5rem)] p-4 md:p-6 max-w-2xl mx-auto pb-20 md:pb-6">
      ...
      <p className="text-slate-500 text-sm mb-4">
        ...
        {header.author_name && ` · 담당 작성: ${header.author_name}`}
      </p>

      <div className="space-y-6 mb-8">
        {WORKER_HYGIENE_CHECKLIST.map((category) => (
          <section
            key={category.title}
