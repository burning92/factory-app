...
type OverallAssessment = "normal" | "observe" | "issue" | null;
...
function overallAssessmentLabel(v: OverallAssessment): string {
...
export default function DailyHoistInspectionListPage() {
  ...
  const fetchLogs = useCallback(async () => {
    ...
    const { data, error } = await supabase
      ...
      .select("id, inspection_date, author_name, overall_assessment, status, approved_at, approved_by_name, reject_reason")
  ...
  const getRowHref = (log: LogRow) => {
    if (log.status === "draft" || log.status === "rejected") return `/daily/hoist-inspection/${log.id}/edit`;
    return `/daily/hoist-inspection/${log.id}`;
  };

  const handleDelete = async (logId: string) => {
    const ok = window.confirm("이 점검기록을 삭제할까요? (항목도 함께 삭제됩니다)");
    if (!ok) return;
    setDeletingId(logId);
    setToast(null);
    const { error } = await supabase.from("daily_hoist_inspection_logs").delete().eq("id", logId);
    setDeletingId(null);
    if (error) {
      setToast({ message: error.message, error: true });
      return;
  ...
  return (
    <div className="min-h-[calc(100vh-3.5rem)] md:min-h-0 p-4 md:p-6 max-w-2xl mx-auto pb-20 md:pb-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-slate-200 font-medium">호이스트 점검기록</span>
      </div>

      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h1 className="text-lg font-semibold text-slate-100">호이스트 점검기록 — 목록</h1>
        <Link
          href="/daily/hoist-inspection/new"
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-slate-700/60 bg-slate-800/30 p-8 text-center">
          <p className="text-slate-500 text-sm mb-4">저장된 점검기록이 없습니다.</p>
          <Link
            href="/daily/hoist-inspection/new"
              ...
              <Link
                ...
                className={`block px-4 py-3 rounded-xl border transition-colors ${
                  log.status === "submitted"
                    ? "border-cyan-600/50 bg-slate-800/80 hover:bg-slate-700/60"
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-xs text-slate-500">종합판단: {overallAssessmentLabel(log.overall_assessment)}</p>
                  </div>
                  <div className="flex items-center gap-1 shrink-0">
                    <button
                      ...
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        window.location.href = `/daily/hoist-inspection/${log.id}/edit`;
                      }}
