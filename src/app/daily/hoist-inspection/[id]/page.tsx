...
import {
  HOIST_INSPECTION_CHECKLIST,
  type HoistOverallAssessment,
} from "@/features/daily/hoistInspectionChecklist";
...
type LogHeader = {
  ...
  overall_assessment: HoistOverallAssessment | null;
  status: string;
...
function overallAssessmentLabel(v: HoistOverallAssessment | null): string {
  if (v === "normal") return "이상없음";
  if (v === "observe") return "관찰필요";
  if (v === "issue") return "이상있음";
  return "—";
}

export default function DailyHoistInspectionViewPage() {
  ...
  const load = useCallback(async () => {
    ...
    const { data: logData, error: logErr } = await supabase
      .from("daily_hoist_inspection_logs")
      .select("*")
      .eq("id", id)
      .maybeSingle();
    if (logErr) {
    ...
    const { data: itemsData, error: itemsErr } = await supabase
      .from("daily_hoist_inspection_log_items")
  ...
  if (error || !header) {
    return (
      <div className="min-h-[calc(100vh-3.5rem)] p-4 md:p-6 max-w-2xl mx-auto">
        ...
        <Link href="/daily/hoist-inspection" className="text-cyan-400 hover:text-cyan-300 text-sm">
          목록으로
        </Link>
      </div>
    );
  ...
  const legacyNotes = items
    .filter((it) => (it.nonconformity_note ?? "").trim() !== "")
    .map((it) => `${it.category} - ${it.question_text}: ${(it.nonconformity_note ?? "").trim()}`);
  ...
  return (
    <div className="min-h-[calc(100vh-3.5rem)] p-4 md:p-6 max-w-2xl mx-auto pb-20 md:pb-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <Link href="/daily/hoist-inspection" className="text-slate-400 hover:text-slate-200 text-sm">
          호이스트 점검기록
        </Link>
        <span className="text-slate-600">/</span>
        <span className="text-slate-200 font-medium">{header.inspection_date}</span>
      </div>
      <h1 className="text-lg font-semibold text-slate-100 mb-1">호이스트 점검기록 — 상세</h1>
      ...
      <p className="text-slate-500 text-sm mb-4">
        종합판단: <span className="text-slate-200">{overallAssessmentLabel(header.overall_assessment)}</span>
      </p>

      <div className="space-y-6 mb-8">
        {HOIST_INSPECTION_CHECKLIST.map((category) => (
      ...
      {!hasCorrective && legacyNotes.length > 0 && (
        <section className="rounded-xl border border-amber-700/50 bg-slate-800/50 p-4 mb-8">
          <h2 className="text-sm font-semibold text-amber-300 mb-2">기존 부적합사항</h2>
          <p className="text-xs text-slate-500 mb-3">이전 구조(item 부적합사항) 데이터입니다.</p>
          <ul className="space-y-2">
            {legacyNotes.map((line, idx) => (
              <li key={idx} className="text-sm text-slate-200 whitespace-pre-wrap">
                {line}
              </li>
            ))}
          </ul>
      ...
      <div className="flex flex-wrap justify-end gap-2 mb-6">
        <Link
          href={`/daily/hoist-inspection/${id}/edit`}
        ...
        <button
          type="button"
          onClick={async () => {
            if (!window.confirm("이 점검기록을 삭제할까요?")) return;
            setActionLoading(true);
            const { error: err } = await supabase.from("daily_hoist_inspection_logs").delete().eq("id", id);
            setActionLoading(false);
            if (err) setError(err.message);
            else router.push("/daily/hoist-inspection");
          }}
          disabled={actionLoading}
