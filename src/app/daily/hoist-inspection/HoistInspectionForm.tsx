...
import {
  ...
  HOIST_OVERALL_ASSESSMENT_OPTIONS,
...
export function HoistInspectionForm({ mode, editLogId }: Props) {
  const { user, profile, viewOrganizationCode } = useAuth();
  const [inspectionDate, setInspectionDate] = useState(todayStr);
  const [overallAssessment, setOverallAssessment] = useState<HoistOverallAssessment | "">("");
  ...
  const autoDeviationText = useMemo(() => {
    const lines: string[] = [];
    HOIST_INSPECTION_CHECKLIST.forEach((cat, ci) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      const { data: logData, error } = await supabase
        ...
        .select(
          "id, inspection_date, overall_assessment, status, corrective_datetime, corrective_deviation, corrective_detail, corrective_remarks, corrective_actor"
        )
        .eq("id", editLogId)
        .maybeSingle();
      if (cancelled || error) {
      ...
      setOverallAssessment(log.overall_assessment ?? "");
      setCorrective({
  ...
  const saveHeader = useCallback(async () => {
    ...
    return logId;
  }, [inspectionDate, orgCode, authorName, overallAssessment, hasAnyX, corrective, mode, currentLogId, user?.id]);
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      if (items.length > 0) {
        const { error: itemsErr } = await supabase.from("daily_hoist_inspection_log_items").insert(items);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (rows.length > 0) {
        const { error: insErr } = await supabase.from("daily_hoist_inspection_log_items").insert(rows);
  ...
  return (
    <div className="min-h-[calc(100vh-3.5rem)] md:min-h-0 p-4 md:p-6 max-w-2xl mx-auto pb-20 md:pb-6">
      ...
      <h1 className="text-lg font-semibold text-slate-100 mb-1">
        {mode === "new" ? "호이스트 점검기록 — 새 작성" : "호이스트 점검기록 — 수정"}
      </h1>
      ...
      <div className="mb-6">
        <p className="block text-sm font-medium text-slate-400 mb-2">종합판단</p>
        <div className="flex flex-wrap gap-2">
          {HOIST_OVERALL_ASSESSMENT_OPTIONS.map((option) => (
            <button
              key={option.value}
              type="button"
              onClick={() => setOverallAssessment(option.value)}
              className={`px-4 py-2 rounded-lg text-sm font-medium border transition-colors ${
                overallAssessment === option.value
      ...
      <div className="space-y-6 mb-8">
        {HOIST_INSPECTION_CHECKLIST.map((category, catIndex) => (
