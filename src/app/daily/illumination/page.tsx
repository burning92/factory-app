...
export default function DailyIlluminationListPage() {
  ...
  const getRowHref = (log: LogRow) => {
    if (log.status === "draft" || log.status === "rejected") return `/daily/illumination/${log.id}/edit`;
    return `/daily/illumination/${log.id}`;
  };
  ...
  return (
    <div className="min-h-[calc(100vh-3.5rem)] md:min-h-0 p-4 md:p-6 max-w-2xl mx-auto pb-20 md:pb-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-slate-200 font-medium">영업장 조도 점검일지</span>
      </div>

      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h1 className="text-lg font-semibold text-slate-100">영업장 조도 점검일지 — 목록</h1>
        <Link
          href="/daily/illumination/new"
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-slate-700/60 bg-slate-800/30 p-8 text-center">
          ...
          <Link href="/daily/illumination/new" className="inline-flex items-center justify-center px-4 py-2 rounded-lg bg-cyan-600 hover:bg-cyan-500 text-white text-sm font-medium">
            새 일지 작성
          </Link>
        </div>
      ) : (
                ...
                <div className="flex items-center pr-2 gap-1 shrink-0">
                  <button
                    ...
                    onClick={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
                      window.location.href = `/daily/illumination/${log.id}/edit`;
                    }}
                  ...
                  <button
                    ...
                    onClick={async (e) => {
                      ...
                      const { error } = await supabase.from("daily_illumination_logs").delete().eq("id", log.id);
                      setDeletingId(null);
