...
export function AirConditioningEquipmentForm({ mode, editLogId }: Props) {
  ...
  const autoDeviationText = useMemo(() => {
    const lines: string[] = [];
    AIR_CONDITIONING_EQUIPMENT_CHECKLIST.forEach((cat, ci) => {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      if (items.length > 0) {
        const { error: itemsErr } = await supabase.from("daily_air_conditioning_equipment_log_items").insert(items);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (rows.length > 0) {
        const { error: insErr } = await supabase.from("daily_air_conditioning_equipment_log_items").insert(rows);
  ...
  return (
    <div className="min-h-[calc(100vh-3.5rem)] md:min-h-0 p-4 md:p-6 max-w-2xl mx-auto pb-20 md:pb-6">
      ...
      <h1 className="text-lg font-semibold text-slate-100 mb-1">
        {mode === "new" ? "공조설비 점검표 — 새 작성" : "공조설비 점검표 — 수정"}
      </h1>
      ...
      <div className="space-y-6 mb-8">
        {AIR_CONDITIONING_EQUIPMENT_CHECKLIST.map((category, catIndex) => (
