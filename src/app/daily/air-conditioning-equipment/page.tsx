...
export default function DailyAirConditioningEquipmentListPage() {
  ...
  const getRowHref = (log: LogRow) => {
    if (log.status === "draft" || log.status === "rejected") return `/daily/air-conditioning-equipment/${log.id}/edit`;
    return `/daily/air-conditioning-equipment/${log.id}`;
  };

  const handleDelete = async (logId: string) => {
    const ok = window.confirm("이 점검표를 삭제할까요? (항목도 함께 삭제됩니다)");
    if (!ok) return;
    setDeletingId(logId);
    setToast(null);
    const { error } = await supabase.from("daily_air_conditioning_equipment_logs").delete().eq("id", logId);
    setDeletingId(null);
    if (error) {
      setToast({ message: error.message, error: true });
      return;
  ...
  return (
    <div className="min-h-[calc(100vh-3.5rem)] md:min-h-0 p-4 md:p-6 max-w-2xl mx-auto pb-20 md:pb-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-slate-200 font-medium">공조설비 점검표</span>
      </div>

      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h1 className="text-lg font-semibold text-slate-100">공조설비 점검표 — 목록</h1>
        <Link
          href="/daily/air-conditioning-equipment/new"
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-slate-700/60 bg-slate-800/30 p-8 text-center">
          <p className="text-slate-500 text-sm mb-4">저장된 점검표가 없습니다.</p>
          <Link
            href="/daily/air-conditioning-equipment/new"
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <button
                      ...
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        window.location.href = `/daily/air-conditioning-equipment/${log.id}/edit`;
                      }}
