...
import { AIR_CONDITIONING_EQUIPMENT_CHECKLIST } from "@/features/daily/airConditioningEquipmentChecklist";
...
export default function DailyAirConditioningEquipmentViewPage() {
  ...
  const load = useCallback(async () => {
    ...
    const { data: logData, error: logErr } = await supabase
      .from("daily_air_conditioning_equipment_logs")
      .select("*")
      .eq("id", id)
      .maybeSingle();
    if (logErr) {
    ...
    const { data: itemsData, error: itemsErr } = await supabase
      .from("daily_air_conditioning_equipment_log_items")
  ...
  if (error || !header) {
    return (
      <div className="min-h-[calc(100vh-3.5rem)] p-4 md:p-6 max-w-2xl mx-auto">
        ...
        <Link href="/daily/air-conditioning-equipment" className="text-cyan-400 hover:text-cyan-300 text-sm">
          목록으로
        </Link>
      </div>
    );
  ...
  return (
    <div className="min-h-[calc(100vh-3.5rem)] p-4 md:p-6 max-w-2xl mx-auto pb-20 md:pb-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <Link href="/daily/air-conditioning-equipment" className="text-slate-400 hover:text-slate-200 text-sm">
          공조설비 점검표
        </Link>
        <span className="text-slate-600">/</span>
        <span className="text-slate-200 font-medium">{header.inspection_date}</span>
      </div>
      <h1 className="text-lg font-semibold text-slate-100 mb-1">공조설비 점검표 — 상세</h1>
      ...
      <div className="space-y-6 mb-8">
        {AIR_CONDITIONING_EQUIPMENT_CHECKLIST.map((category) => (
      ...
      <div className="flex flex-wrap justify-end gap-2 mb-6">
        <Link
          href={`/daily/air-conditioning-equipment/${id}/edit`}
        ...
        <button
          type="button"
          onClick={async () => {
            if (!window.confirm("이 점검표를 삭제할까요?")) return;
            setActionLoading(true);
            const { error: err } = await supabase.from("daily_air_conditioning_equipment_logs").delete().eq("id", id);
            setActionLoading(false);
            if (err) setError(err.message);
            else router.push("/daily/air-conditioning-equipment");
          }}
          disabled={actionLoading}
      ...
      <div className="flex justify-end">
        <Link
          href="/daily/air-conditioning-equipment"
          className="px-4 py-2 rounded-lg border border-slate-600 text-slate-300 hover:bg-slate-700/50 text-sm"
        >
          목록으로
        </Link>
