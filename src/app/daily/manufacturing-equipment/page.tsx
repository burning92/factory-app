...
export default function DailyManufacturingEquipmentListPage() {
  ...
  const getRowHref = (log: LogRow) => {
    ...
    return `/daily/manufacturing-equipment/${log.id}`;
  };
  ...
  const handleDelete = async (logId: string) => {
    ...
    setDeletingId(logId);
    setToast(null);
    const { error } = await supabase.from("daily_manufacturing_equipment_logs").delete().eq("id", logId);
    setDeletingId(null);
    if (error) {
      setToast({ message: error.message, error: true });
      return;
  ...
  return (
    <div className="min-h-[calc(100vh-3.5rem)] md:min-h-0 p-4 md:p-6 max-w-2xl mx-auto pb-20 md:pb-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-slate-200 font-medium">제조설비 점검표</span>
      </div>
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <button
                      ...
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        window.location.href = `/daily/manufacturing-equipment/${log.id}/edit`;
                      }}
