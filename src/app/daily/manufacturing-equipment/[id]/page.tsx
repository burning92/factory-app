  ...
  if (error || !header) {
    return (
      <div className="min-h-[calc(100vh-3.5rem)] p-4 md:p-6 max-w-2xl mx-auto">
        ...
        <Link href="/daily/manufacturing-equipment" className="text-cyan-400 hover:text-cyan-300 text-sm">
          목록으로
        </Link>
      </div>
    );
  ...
  return (
    <div className="min-h-[calc(100vh-3.5rem)] p-4 md:p-6 max-w-2xl mx-auto pb-20 md:pb-6">
      ...
      <div className="flex flex-wrap justify-end gap-2 mb-6">
        <Link
          href={`/daily/manufacturing-equipment/${id}/edit`}
        ...
        <button
          ...
          onClick={async () => {
            ...
            setActionLoading(true);
            const { error: err } = await supabase.from("daily_manufacturing_equipment_logs").delete().eq("id", id);
            setActionLoading(false);
            if (err) setError(err.message);
            else router.push("/daily/manufacturing-equipment");
          }}
          disabled={actionLoading}
