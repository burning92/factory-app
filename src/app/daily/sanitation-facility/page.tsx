...
export default function DailySanitationFacilityListPage() {
  ...
  const getRowHref = (log: LogRow) => {
    if (log.status === "draft" || log.status === "rejected") return `/daily/sanitation-facility/${log.id}/edit`;
    if (log.status === "approved" && isManager) return `/daily/sanitation-facility/${log.id}/edit`;
    return `/daily/sanitation-facility/${log.id}`;
  };

  const getEditHref = (log: LogRow) => {
    if (log.status === "submitted") return `/daily/sanitation-facility/${log.id}`;
    return `/daily/sanitation-facility/${log.id}/edit`;
  };
  ...
  const handleDelete = async (logId: string) => {
    ...
    setDeletingId(logId);
    setToast(null);
    const { error } = await supabase.from("daily_sanitation_facility_logs").delete().eq("id", logId);
    setDeletingId(null);
    if (error) {
      setToast({ message: error.message, error: true });
      return;
