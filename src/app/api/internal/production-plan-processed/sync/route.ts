...
import type { ProcessedPlanSyncPayload } from "@/features/production/plan/processedPlanTypes";
import { normalizeProcessedPlanRows } from "@/features/production/plan/normalizeProcessedPlanRows";

const SYNC_NAME = "production_plan_processed";
...
export async function POST(req: NextRequest) {
  ...
  let body: ProcessedPlanSyncPayload;
  try {
    body = (await req.json()) as ProcessedPlanSyncPayload;
  } catch {
  ...
  const { rows: normalized, filtered } = normalizeProcessedPlanRows(rawRows);
...
export async function GET() {
  return NextResponse.json({ ok: true, route: "production-plan-processed-sync" });
}
