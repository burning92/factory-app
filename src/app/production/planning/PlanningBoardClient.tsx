...
export default function PlanningBoardClient() {
  ...
  const nearExpiryRows = useMemo(() => data?.nearExpiryRows ?? [], [data?.nearExpiryRows]);

  useEffect(() => {
    if (nearExpiryRows.length === 0) return;
    try {
      const seen = window.sessionStorage.getItem(PLANNING_NEAR_EXPIRY_SEEN_STORAGE_KEY);
      if (seen === "1") return;
      window.sessionStorage.setItem(PLANNING_NEAR_EXPIRY_SEEN_STORAGE_KEY, "1");
      setNearExpiryOpen(true);
    } catch {
      setNearExpiryOpen(true);
    }
            ...
            ? createPortal(
                <>
                  <button
                    ...
                    onClick={() => setNearExpiryOpen(false)}
                  />
                  <div className="fixed left-1/2 top-1/2 z-[370] w-[min(94vw,46rem)] max-h-[82vh] -translate-x-1/2 -translate-y-1/2 overflow-hidden rounded-xl border border-amber-500/40 bg-space-900 shadow-2xl shadow-black/50">
                    <div className="flex items-start justify-between gap-3 border-b border-slate-700 px-4 py-3">
                      <div>
                        <p className="text-xs text-amber-300/90">플래닝 진입 알림</p>
                        <h3 className="text-lg font-semibold text-amber-100">소비기한 30일 이내 원료</h3>
                        <p className="mt-1 text-xs text-slate-400">
                          제외 품목(포장재/박스류, 설탕/소금류 등)과 품목명 "바질"은 자동 제외되었습니다.
                        </p>
                        <p className="mt-1 text-[11px] text-slate-500">
                          ※ 본 알림은 마지막으로 불러온 이카운트 재고 데이터를 기준으로 표시됩니다.
                        </p>
                      </div>
                      <button
                        type="button"
                    ...
                    <div className="max-h-[58vh] overflow-y-auto px-4 py-3">
                      <table className="w-full text-xs sm:text-sm">
                        <thead className="sticky top-0 bg-space-900 text-slate-400">
